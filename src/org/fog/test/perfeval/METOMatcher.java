...
public class METOMatcher {
     ...
     * with deadline-aware outage detection
     * 
     ...
     * @param taskDeadlines Deadline of each task (in milliseconds)
     * @param executionTimesOnFN Estimated execution time of each task on each FN
     * @return Matching map: task index -> matched FN index
     */
    public static Map<Integer, Integer> runMatching(
        Map<Integer, List<Integer>> taskPrefs,
        Map<Integer, List<Integer>> fnPrefs,
        Map<Integer, Integer> fnQuotas,
        Map<Integer, Double> taskDeadlines,
        Map<Integer, Map<Integer, Double>> executionTimesOnFN) {
        ...
        Set<Integer> freeTasks = new HashSet<>(taskPrefs.keySet()); // Initially, all tasks are unmatched.
        ...
        while (!freeTasks.isEmpty()) {
            Iterator<Integer> iter = freeTasks.iterator(); // Keeps iterating until all tasks are matched or have exhausted preferences.
            while (iter.hasNext()) {
                int task = iter.next();
                List<Integer> preferences = taskPrefs.get(task);
                if (taskNextProposal.get(task) >= preferences.size()) {
                    iter.remove(); // If a task has exhausted all preferences, remove it from the loop.
                    continue;
                }

                int fn = preferences.get(taskNextProposal.get(task)); // The FN this task is proposing to.
                taskNextProposal.put(task, taskNextProposal.get(task) + 1);

                fnToTasks.get(fn).add(task); // Tentatively assign task to FN.
                ...
                while (ranked.size() > fnQuotas.get(fn)) {
                    int worst = ranked.get(ranked.size() - 1); // Least preferred task
                    ranked.remove(ranked.size() - 1);
                    fnToTasks.get(fn).remove(worst); // Remove from tentative assignment
                    fnRejected.get(fn).add(worst); // Add to rejected list
                }
            }

            // Reassign freeTasks for next round
            freeTasks.clear();
            for (Map.Entry<Integer, Set<Integer>> entry : fnRejected.entrySet()) {
                freeTasks.addAll(entry.getValue()); // Tasks rejected in this round will propose again
        ...
        // ✅ Deadline Check and Outage Report
        int totalTasks = taskPrefs.size();
        int outageCount = 0;
        double totalexecTime= 0;

        for (Map.Entry<Integer, Integer> entry : taskToFN.entrySet()) {
            int taskId = entry.getKey();
            int fnId = entry.getValue();

            double execTime = executionTimesOnFN.get(taskId).get(fnId);
            totalexecTime = totalexecTime + execTime; 
            double deadline = taskDeadlines.get(taskId);

            if (execTime > deadline) {
                outageCount++;
//                System.out.println("⚠️ Task " + taskId + " assigned to FN " + fnId +
//                                   " missed deadline. ExecTime: " + String.format("%.2f", execTime) + 
//                                   " > Deadline: " + deadline );
            }
        ...
        System.out.println("Tasks Missed Deadline : " + outageCount);
        System.out.println("Outage Percentage     : " + String.format("%.2f", (outageCount * 100.0) / totalTasks) + " %");
        System.out.println("Average Execution Time : " + totalexecTime/totalTasks);
