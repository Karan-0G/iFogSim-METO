...
public class DecisionMatrixUtil {
    ...
    public static Map<Task, double[][]> constructTaskMatrices(List<Task> tasks, List<FogNodeWrapper> fogNodes) {
        ...
        for (Task task : tasks) {
            ...
            for (int i = 0; i < fogNodes.size(); i++) {
                ...
                double gain = computeChannelGain(fn.id); // Dummy value: assume fixed gain or provide distance
                double uplinkRate = computeUplinkRate(task.inputSize, gain);//uplinkRate simulates bandwidth between device and fog node.
                double txTime = task.inputSize / uplinkRate;//txTime: Time to send task data. transmission time
                double compTime = (double) task.cycles / fn.capacity;
    ...
    public static Map<FogNodeWrapper, double[][]> constructFogNodeMatrices(List<Task> tasks, List<FogNodeWrapper> fogNodes) {
        ...
        for (FogNodeWrapper fn : fogNodes) {
            ...
            for (int j = 0; j < tasks.size(); j++) {
                ...
                double gain = computeChannelGain(fn.id); // Dummy again
                double uplinkRate = computeUplinkRate(task.inputSize, gain);
                double txTime = task.inputSize / uplinkRate;
                double rxTime = task.outputSize / uplinkRate;
                double compTime = (double) task.cycles / fn.capacity;

                double fnEnergy = fn.txPower * (txTime + rxTime) + compTime * fn.powerBusy;//fnEnergy: Total energy used by the fog node.
        ...
        for (int i = 0; i < criteria; i++) {
            for (int j = 0; j < criteria; j++) {
                ...
                for (int k = 0; k < alternatives; k++) {
                    ...
                    denom1 += normalized[k][i] * normalized[k][i];
    ...
    public static double computeChannelGain(double distance) {
        double pathLossDb = 38.02 + 20 * Math.log10(distance);
        return Math.pow(10, -pathLossDb / 10);
    }

    public static double computeUplinkRate(int dataSize, double gain) {
